<template>
  <section class="space-y-6">
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <article v-for="role in roles || []" :key="role.id" class="surface rounded-2xl p-4">
        <div class="flex items-start justify-between gap-3">
          <div>
            <h3 class="font-display text-xl">{{ role.name }}</h3>
            <p class="text-xs text-muted">{{ role.key }}{{ role.is_system ? ' · system' : '' }}</p>
          </div>
          <AppButton v-if="auth.can('roles:update') && !role.is_system" variant="secondary" @click="edit(role)">Edit</AppButton>
        </div>
        <div class="mt-3 flex flex-wrap gap-2">
          <span v-for="item in role.role_permissions" :key="item.permission.key" class="chip">
            {{ item.permission.key }}
          </span>
        </div>
      </article>
