<template>
  <div class="mx-auto max-w-xl px-4 py-12">
    ...
    <div v-if="result" class="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <dl class="mt-4 grid gap-2 text-sm">
        ...
        <div class="flex justify-between"><dt>Category</dt><dd>{{ result.category || '—' }}</dd></div>
      ...
      <p v-if="result.resolution_summary" class="mt-4 text-sm text-slate-600">{{ result.resolution_summary }}</p>
    </div>
  </div>
</template>
...
import { trackPublicComplaint } from '@/services/complaint.service';
